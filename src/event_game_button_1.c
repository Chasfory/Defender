...
{
    if (tower_position[0][struct_game->j].tower_in_game == DIS_MENU_LEVEL_UP) {
        for (int i = 0; i < 2; i++) {
        ...
        struct_game, tower_position[0], i);
        }
    }
}
...
{
    if (tower_position[0][struct_game->j].tower_in_game == DIS_MENU_TOWER) {
        for (int i = 0; i < 4; i++) {
        ...
        every_button_display_minus(every_button[DIS_MENU_TOWER], struct_game,
        tower_position[0], i);
        }
    } else
        event_1_bis(struct_game, every_button, tower_position, mouse_click);
}
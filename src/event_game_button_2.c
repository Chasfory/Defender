...
{
    if (tower_position[1][struct_game->j].tower_in_game == DIS_MENU_LEVEL_UP) {
        for (int i = 0; i < 2; i++) {
        ...
        struct_game, tower_position[1], i);
        }
    }
}
...
{
    if (tower_position[1][struct_game->j].tower_in_game == DIS_MENU_TOWER) {
        for (int i = 0; i < 4; i++) {
        ...
        event_2_bis(struct_game, every_button, tower_position, mouse_click);
}